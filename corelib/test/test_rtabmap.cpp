...
SensorData makeFeaturesData(int id, int featSlot = 0, int k = 8)
{
	...
	std::vector<cv::Matx33f> pts3Covariances;
	...
	for(int i = 0; i < k && i < 8; ++i)
	{
		...
		pts3Covariances.push_back(cv::Matx33f::eye()*0.01f);
	}
		...
		std::mt19937 & rng)
{
	...
	for(const auto & lm : landmarks)
	{
		...
		pts3Covariances.emplace_back(cv::Matx33f::eye()*0.01f);
	...
	data.setFeatures(kpts, pts3, desc, pts3Covariances);
	return data;
}

...
TEST(MemoryTest, ConvertToIntermediateClearsAllPayloadsByDefault)
{
	...
	std::vector<cv::Matx33f> pts3Covariances(2, cv::Matx33f::eye()*0.01f);
	cv::Mat descriptors = cv::Mat::ones(2, 8, CV_32F);
	s->sensorData().setFeatures(kpts, pts3, descriptors, pts3Covariances);

	std::multimap<int, int> wordIds;
	wordIds.insert(std::make_pair(11, 0));
	wordIds.insert(std::make_pair(22, 1));
	s->setWords(wordIds, kpts, pts3, pts3Covariances, descriptors);
...
int buildDictionaryDb(const std::string & dbPath)
{
	...
	for(int frame = 0; frame < kFrames; ++frame)
	{
		...
		std::vector<cv::Matx33f> pts3Covariance(kKeypointsPerFrame, cv::Matx33f::eye()*0.01f);
		...
		data.setFeatures(kpts, pts3, descriptors, pts3Covariance);
...
TEST(MemoryTest, SetDummyDictionaryDisablesItselfWhenDatabaseHasNoWords)
{
	...
	for(int pass = 0; pass < 2; ++pass)
	{
		...
		std::vector<cv::Matx33f> pts3Covariances(kKeypoints, cv::Matx33f::eye()*0.01f);
		...
		data.setFeatures(kpts, pts3, descriptors, pts3Covariances);
		const cv::Mat covariance = cv::Mat::eye(6, 6, CV_64FC1) * 0.01;
...
TEST(MemoryTest, ForgetTransfersBasedOnWordCountInWordRegime)
{
	...
	auto makeData = [&](int frameIndex) {
		...
		std::vector<cv::KeyPoint> kpts(kKeypointsPerFrame, cv::KeyPoint(1.f, 1.f, 1.f));
		std::vector<cv::Point3f> pts3(kKeypointsPerFrame, cv::Point3f(0.f, 0.f, 1.f));
		std::vector<cv::Matx33f> pts3Covariances(kKeypointsPerFrame, cv::Matx33f::eye()*0.01f);
	...
	// -kKeypointsPerFrame + 1 = -4 words.
	{
		...
		std::vector<cv::Matx33f> onePt3Covariance(1, cv::Matx33f::eye()*0.01f);
		...
		extraData.setFeatures(oneKpt, onePt3, descriptors, onePt3Covariance);
...
TEST_F(MemoryFixture, GetNodeWordsReturnsSignaturesWords)
{
	...
	memory_->getNodeWordsAndGlobalDescriptors(id, words, wordsKpts, words3, words3Covariances, wordsDescriptors, globalDescriptors);
